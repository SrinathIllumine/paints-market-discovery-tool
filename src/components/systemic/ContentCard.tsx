...
export function ContentCard({
  ...
  return (
    <button
      ...
      className={cn(
        "group flex w-full items-start gap-3.5 rounded-xl border border-border bg-card p-3 text-left transition-all",
      ...
      >
        {hasImage ? (
          ...
          <div className={cn("flex h-full w-full items-center justify-center", tone === "navy" ? "text-navy" : "text-critical")}>
            {item.image ? <ImageOff className="h-5 w-5 opacity-40" /> : <FallbackIcon className="h-7 w-7 opacity-70" />}
          </div>
        )}
      </div>
      <p className="min-w-0 flex-1 self-center text-sm leading-relaxed text-foreground">{item.text}</p>
    </button>
  );
}
