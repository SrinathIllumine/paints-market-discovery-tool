...
import { Map as MapIcon, ClipboardList, Lightbulb } from "lucide-react";
...
const items = [
  ...
  { to: "/sales-enablement", icon: Lightbulb, label: "Sales Enablers", disabled: false },
] as const;

export function BottomNav() {
  const path = useRouterState({ select: (s) => s.location.pathname });
  return (
    <nav className="sticky bottom-0 z-30 grid grid-cols-3 border-t border-border bg-card/95 backdrop-blur md:rounded-b-3xl">
      {items.map((it) => {
        const Icon = it.icon;
        if (it.disabled) {
          return (
